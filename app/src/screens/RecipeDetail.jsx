...
const T = {
  ...
  greenSoft: '#E7EFE9',
  tomato: '#D64525', muted: '#5E6E64', line: '#E4E0D6',
}
    ...
    <div style={{
      ...
      <div style={{ padding: '16px 20px 0' }}>
        ...
        <div style={{ display: 'flex', gap: 10, marginTop: 14 }}>
          {[
            ['Coverage', `${recipe.displayCoverageScore}%`],
            ['Missing', recipe.missingIngredients.length],
            ['Needs', `${recipe.ingredients.length} items`],
          ].map(([label, value]) => (
            <div key={label} style={{
              flex: 1, background: '#fff', border: `1px solid ${T.line}`,
              borderRadius: 12, padding: '10px 8px', textAlign: 'center',
            }}>
              <div style={{ fontWeight: 700, fontSize: 15, color: T.ink }}>{value}</div>
        ...
        <div style={{ display: 'grid', gap: 6 }}>
          {recipe.ingredients.map(ingredient => {
            const have = matched.has(ingredient)
            return (
              <div key={ingredient} style={{ display: 'flex', alignItems: 'center', gap: 8, fontSize: 14 }}>
                ...
                <span style={{ color: have ? T.ink : T.muted, textTransform: 'capitalize' }}>
                  {displayIngredientLabel(ingredient)}
                </span>
              </div>
            )
          })}
